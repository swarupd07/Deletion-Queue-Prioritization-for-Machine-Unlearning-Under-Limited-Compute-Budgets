...
METHODS = ("sis", "cooks", "leverage", "residual", "random")
LABELS = {
    "sis": "SIS",
    "cooks": "Cook's",
    "leverage": "Leverage",
...
def main():
                ...
                rows.append(
                    {
                        ...
                        "scoring_completion_rate": m[
                            "scoring_completed_mean"
                        ][j],
                        ...
                        "score_sec_mean": (
                            ds["timing"][method]["score_sec_mean"]
                            if method != "random"
                            else 0.0
                        ),
                        "sort_sec_mean": (
                            ds["timing"][method]["sort_sec_mean"]
                            if method != "random"
                            else 0.0
                        ),
                        "priority_total_sec_mean": (
                            ds["timing"][method]["priority_total_sec_mean"]
                            if method != "random"
                            else 0.0
                        ),
    ...
    fig.suptitle(
        "Influence-score prioritization under an equal total compute budget\n"
        "Mean +/- 1 standard deviation across queue realizations",
        fontsize=15,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(args.output_dir / "equal_compute_budget_all_methods.png", dpi=220)
    fig.savefig(
        args.output_dir / "equal_compute_budget_all_methods.pdf",
        bbox_inches="tight",
    )
    plt.close(fig)

    table_path = args.output_dir / "equal_compute_budget_all_methods_table.csv"
