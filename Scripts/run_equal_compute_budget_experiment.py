...
For every queue realization, the experiment measures:
  * the wall-clock cost of computing SIS for every queued request;
  * the wall-clock cost of sorting those scores;
  * the average wall-clock cost of one exact RLS.unlearn() operation.

At each fixed total compute budget, SIS pays its scoring + sorting cost first
and spends only the remainder on exact deletions. FIFO and Random pay no
scoring cost and spend the whole budget on exact deletions. All methods are
therefore charged against the same wall-clock budget. Evaluation time and the
one-off construction of the fully-deleted target are deliberately excluded.
"""
...
def sis_scores_for_queue(model: RLS, queue: np.ndarray) -> np.ndarray:
    """Compute SIS only for the Q requested rows, not all n training rows."""
    ...
    return residual**2 * px_norm_sq / denom**2
...
def run_realization(
    ...
    _ = sis_scores_for_queue(model, queue)
    warm = model.copy()
    warm.unlearn(int(queue[0]))

    sis_scores, t_score = median_timed_call(
        lambda: sis_scores_for_queue(model, queue), timing_reps
    )
    sis_positions, t_sort = median_timed_call(
        lambda: np.argsort(-sis_scores, kind="stable"), timing_reps
    )
    t_priority = t_score + t_sort
    ...
    orders = {
        "sis": queue[sis_positions],
        "fifo": queue.copy(),
        "random": np.random.default_rng(seed).permutation(queue),
    }
    overhead = {"sis": t_priority, "fifo": 0.0, "random": 0.0}

    out = {
        "queue_size": int(Q),
        "sis_score_sec": t_score,
        "sis_sort_sec": t_sort,
        "sis_priority_total_sec": t_priority,
    ...
    for method in METHODS:
        remaining = np.maximum(0.0, budgets_sec - overhead[method])
        ...
        charged = overhead[method] + ks * t_unlearn_avg
        out["methods"][method] = {
            "n_deleted": ks.tolist(),
...
def aggregate_realizations(realizations):
    ...
    for method in METHODS:
        out["methods"][method] = {}
        for metric in ("n_deleted", "charged_compute_sec", "progress", "test_mse"):
            values = np.asarray(
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("equal_compute_budget_results.json")
    )
